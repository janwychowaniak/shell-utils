...
| `jw_functions__files.sh` | Filesystem oversight cockpit — read-only orientation/profiling (size, recency, tree, search, posture, hygiene) + a print-only backup helper; fully 🟢, server-safe, oversight-first (`jwfiles_*`) |
| `jw_functions__fs.sh` | File-naming manipulation — batch rename/despace/unpolish, etc. (`jwodspacjacz*`, `jwbatchmv`, legacy) |
