...
- **Mutating commands with a natural default** (`jwgit_push` → current branch, `jwgit_pull`, `jwgit_fetch` → origin, `jwgit_clean` → dry-run, `jwgit_gc`) follow the same rule: no-args performs the default and `-h`/`--help` is help-only — gating the default behind `-h` makes the help lie the same way ("`jwgit_push` # Push current branch" while no-args only printed help). Guard the write: a bare **write** default confirms first (`[y/N]` — push/pull); a **read-only or safe** default runs without a prompt (fetch only updates remote-tracking refs; clean's bare form is a dry run, `-f` still required to delete). Only genuinely required-arg commands keep usage-on-no-args.
...
- **Data-driven / range output — align dynamically, don't hardcode a width.** The fixed-width `__<area>_kv__` above suits small static blocks; for output built by ranging over unknown data (labels, env, mounts, networks) a hardcoded `%-Ns` overflows the moment a key runs long. Align to the data instead: **key→value** via a longest-key+2 two-pass awk helper (`__jwdocker_kvalign__`), **multi-field records** via `column -t` (`__jwdocker_table__` — emit tab-separated rows + a header row, indent 2). Both print nothing for an empty set (no lone header). **Gotcha:** `docker inspect -f` / `--format` strips leading whitespace from the template *and* appends a trailing newline — so indent list rows with `… | sed 's/^/  /'`, not spaces inside the template, and let the helper swallow the phantom blank. (And don't print the raw value of a struct/slice field — e.g. `.NetworkSettings.Ports` — range into its parts; the bare `{{$value}}` dumps Go's `[{0.0.0.0 5678} …]`.)
